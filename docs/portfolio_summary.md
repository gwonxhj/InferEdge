...
| Can reviewers follow the Lab report quickly? | Lab `Review Path` section plus `Validated Review Path` gate summary, including `review_path_section: short Review Path section rendered`, for the README -> Lab report -> gate summary reading order |
