...
|---|---|---|---|
| Agent runtime operation smoke | `bash scripts/demo_agent_runtime_e2e.sh` or the Agent Runtime EdgeEnv stage inside `bash scripts/smoke_all.sh` | File-based agent manifest, Runtime `result.agent`, Orchestrator operation evidence, AIGuard runtime warning evidence, Lab agent-runtime report, optional remote-dispatch starter markers, and optional EdgeEnv run preservation identity/details labels are connected in one generated bundle | Runtime regression comparability, telemetry-history replay, or CI artifact bundle completeness |
