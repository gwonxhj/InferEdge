...
2026-06-23 KST에 Jetson readiness preflight가 `--edgeenv-run-evidence`로
통과한 뒤 current-main sustained capture를 새로 실행했습니다.
...
이 repeat는 sustained device-local evidence path가 current entrypoint에서도
Orchestrator, EdgeEnv, AIGuard, Lab artifact를 함께 보존한다는 점을 보여줘
포트폴리오 증거력을 강화합니다. 다만 여전히 runtime-operation smoke
evidence이며 production service operation이나 sustained thermal endurance
validation은 아닙니다.
