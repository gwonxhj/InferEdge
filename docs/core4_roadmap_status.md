...
| AIGuard Detector Matrix Depth | InferEdgeAIGuard PR #107 adds deterministic detection disappearance evidence for candidate zero detections against non-empty baseline evidence | Complete for the disappearance case; per-class drift and calibration drift remain later candidates |
...
- AIGuard disappearance evidence adds a deterministic failure mode without LLM
  root-cause guessing.
...
| AIGuard per-class drift | Adds finer diagnosis for class-specific output changes | Needs carefully bounded deterministic evidence and fixture coverage |
| AIGuard calibration drift | Explains confidence distribution shifts | Should not become an ML root-cause claim or replace Lab deployment decision rules |
