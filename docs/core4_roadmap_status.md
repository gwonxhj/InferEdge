...
InferEdgeOrchestrator PR #118 adds a supplemental `pressure_window` summary to
the sustained operation timeline so reviewers can see overload-threshold
...
decision owner.
InferEdgeEnv PR #158 preserves those policy-pressure reason counts in runtime
telemetry history inspection, Runtime Intelligence Lab handoff summaries, and
CLI output, keeping the Orchestrator first-read scheduler reason distribution
visible through EdgeEnv without turning it into an EdgeEnv regression gate or
...
InferEdgeLab PR #378 surfaces the same optional AIGuard
`edgeenv_orchestrator_pressure_window_summary` evidence in the Lab-owned
...
InferEdgeOrchestrator PR #120 adds an additive
`scenario_coverage` block to the sustained operation timeline and prints a
`scenario-coverage` / `scenario_coverage` first-read line from the CLI and
EdgeEnv feed contract checker, so reviewers can see observed cycles, timeline
sample counts, and producer sources before opening the full JSON.
InferEdgeEnv PR #159 validates and preserves that `scenario_coverage` block in
runtime telemetry history inspection, Runtime Intelligence Lab handoff
summaries, and CLI output, exposing `scenario_coverage_run_ids` /
`orchestrator_scenario_coverage_run_ids` while keeping it optional reviewer
navigation context rather than an EdgeEnv regression gate or deployment
decision.
...
- Orchestrator sustained runs now expose `scenario_coverage` with observed
  scheduler cycles, queue/latency/runtime-event sample counts, producer source
  coverage, and Lab ownership boundaries, making the source evidence scope
  easier to audit before downstream preservation.
- EdgeEnv now carries that `scenario_coverage` context through telemetry
  history inspection and Lab handoff summaries, preserving observed scenario
  scope as reviewer navigation without changing comparability or deployment
  decision ownership.
...
- Lab Runtime Intelligence reports now show that optional pressure-window
  evidence row and validate it through bundle/artifact/CI gates, so the
  Orchestrator -> EdgeEnv -> AIGuard -> Lab overload-window chain is visible in
  the final Lab-owned reviewer surface without becoming a required AIGuard
  handoff contract.
