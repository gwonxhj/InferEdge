...
외부 reviewer에게는 entrypoint README에서 시작하고 `## Docs & Review Path`
순서를 유지합니다.

1. `README.md`
2. `docs/ecosystem_1page.ko.md`
3. 이 요약
4. `docs/pipeline_map.ko.md`
5. `docs/final_submission_rehearsal.md`
6. `docs/agent_runtime_e2e_demo.ko.md`
7. `docs/interview_narrative.ko.md`
8. 현재 evidence reports: Lab Local Studio / reports, Runtime Jetson reports,
   AIGuard detector validation matrix, Jetson sustained evidence snapshots
...
`repos.lock`은 `clone_all.sh`가 사용하는 pinned smoke snapshot입니다.
`repos.yaml`은 ecosystem role map과 curated Orchestrator sample handoff 같은
historical supporting reference를 보존합니다.
