...
| Locked smoke repository map | `repos.lock`, `repos.yaml`, `scripts/clone_all.sh --locked`, and `scripts/smoke_all.sh` | `repos.lock` is the pinned smoke snapshot; `repos.yaml` remains the ecosystem role map and historical supporting-reference record, not the locked clone source |
...
policy-pressure alignment commits included. `repos.yaml` remains a role map and
