...
Runtime Operation first-read path는 이렇게 말합니다. Orchestrator가
`operation-risk` first-read를 보여주고, EdgeEnv가
`operation_risk_rollup_first_reads`를 보존하며, Lab Runtime Intelligence report가
`first_read=review_operation_risk_context`를 렌더링합니다. 이는 risk context를
빠르게 보기 위한 reviewer navigation이며 Lab-owned decision authority를
바꾸지 않습니다.
...
- "registry는 quick-scan navigation metadata이며 production runtime operation
  proof가 아니다"
- "operation-risk first-read는 Orchestrator, EdgeEnv, Lab을 가로질러 reviewer가
  먼저 볼 risk context를 알려주지만 decision owner를 바꾸지 않는다"
