...
| 4 | Lab `Review Path` section, `Review path` note, `Fast path` line, `Validated Review Path` gate summary | README -> Lab report -> gate summary 순서를 reviewer가 그대로 따라가게 하며, gate는 `review_path_section: short Review Path section rendered`, `review_path_fast_path: readable Review Path fast path rendered`, `review_path_artifact_gate_summary: artifact gate summary reference row validated`를 보존하되 CI, AIGuard, Orchestrator가 report owner처럼 보이지 않게 유지 |
| 5 | generated registry의 `Operation Quick Scan Summary` | wide table을 열기 전 compact queue pressure, depth(`max_total_queue_depth`), deadline miss, fallback count, preservation label 확인 |
| 6 | `00_evidence_index.md` / `.json` 및 detailed registry row | Jetson/device-local preservation context, `identity=jetson_device_local_preservation`, `lab_preservation=present`, `raw_marker=reviewer_focus_operation_quick_scan`이 navigation metadata로 남는지 확인 |
