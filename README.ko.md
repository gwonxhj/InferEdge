...
| 5 | generated registry의 `Operation Quick Scan Summary` | wide table을 열기 전 queue pressure, `max_total_queue_depth`, deadline miss, fallback count, Lab quick-scan raw marker 확인 |
| 6 | `00_evidence_index.md` / `.json` | Jetson/device-local preservation context, `identity=jetson_device_local_preservation`, `lab_preservation=present`, `raw_marker=reviewer_focus_operation_quick_scan`이 navigation metadata로 남는지 확인 |
