...
| 3 | Lab `Review Path` section, `Review path` note, `Validated Review Path` gate summary | README -> Lab report -> gate summary 순서를 reviewer가 그대로 따라가게 하며, gate는 `review_path_section: short Review Path section rendered`를 보존하되 CI, AIGuard, Orchestrator가 report owner처럼 보이지 않게 유지 |
