...
| 3 | Lab `Review path` note와 `Validated Review Path` gate summary | README -> Lab report -> gate summary 순서를 reviewer가 그대로 따라가게 하되 CI, AIGuard, Orchestrator가 report owner처럼 보이지 않게 유지 |
