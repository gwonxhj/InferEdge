...
| 단계 | 먼저 볼 것 | 의미 |
|---|---|---|
| 1 | `runtime_intelligence_bundle_manifest_gate_summary.md` | `Orchestrator -> EdgeEnv -> AIGuard -> Lab` bundle과 owner boundary가 유지되는지 확인 |
| 2 | `runtime_anomaly_summary.md` / `.html` | Lab-owned Runtime Intelligence Risk Summary, duration traceability, operation quick scan 확인 |
| 3 | Lab `Review Path` section, `Review path` note, `Fast path` line, `Validated Review Path` gate summary | README -> Lab report -> gate summary 순서를 reviewer가 그대로 따라가게 하며, gate는 `review_path_section: short Review Path section rendered`와 `review_path_fast_path: readable Review Path fast path rendered`를 보존하되 CI, AIGuard, Orchestrator가 report owner처럼 보이지 않게 유지 |
| 4 | generated registry의 `Operation Quick Scan Summary` | wide table을 열기 전 queue pressure, `max_total_queue_depth`, deadline miss, fallback count, Lab quick-scan raw marker 확인 |
| 5 | `00_evidence_index.md` / `.json` | Jetson/device-local preservation context, `identity=jetson_device_local_preservation`, `lab_preservation=present`, `raw_marker=reviewer_focus_operation_quick_scan`이 navigation metadata로 남는지 확인 |
| 6 | Remote fallback rows | `Remote fallback starter evidence`를 보존하되 production remote execution으로 해석하지 않음 |
